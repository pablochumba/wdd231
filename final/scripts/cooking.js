...
const list = document.querySelector('#cooking-list');
...
async function init() {
  allExperiences = (await getExperiences()).filter((item) => item.category === 'cooking');

  if (!allExperiences.length) {
    list.innerHTML = '<p class="empty-state">No cooking experiences are available right now.</p>';
    return;
  }
